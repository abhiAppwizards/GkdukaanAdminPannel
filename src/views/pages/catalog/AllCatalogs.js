...
const getAllCatalogs = async() =>{
  try {
    const response = await axios.get(`${config.baseURL}/vendor/catalog`,{
      "headers": {
        "authorization": token
      }
    })
  ...
  return (
    <div className="">
      ...
      <div className="mb-8 rounded-lg bg-white bg-light -3 md:p-8">
        <h1 className="font-bold text-xl mb-4"> Your All Catalogs Here..</h1>
            ...
            <tbody>
              {currentProducts?.map((catalog) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-center">{catalog.name}</td>
                  <td className="px-6 py-4 text-center">{catalog.full_details}</td>
                  {/* <td className="text-center">
                    <div className="flex justify-center items-center rtl:space-x-reverse">
                      ...
                      {catalog.in_stock ? 'In Stock' : 'Out of Stock'}
                      </span>
                    </div>
                  </td> */}
